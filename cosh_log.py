...
def smooth_signal(input,len = 25,window = "blackman"):
  ...
  smooth_input = smooth(input,len,window)
  idx = int(np.floor(len/2))
  return smooth_input[idx:-idx]
...
def smooth_frames(signal, frame_len,overlap):
    num = int(np.floor(len(signal)/frame_len)-1)
    for h in range(1,num):
        if (overlap<0.5):
            signal[int(h * frame_len - frame_len*overlap) - 100:int(h*frame_len - frame_len*overlap) + 100] = smooth_signal(signal[int(h*frame_len - frame_len*overlap) - 100:int(h*frame_len - frame_len*overlap) + 100], 7)
            signal[int(h*frame_len)-100:int(h*frame_len)+100] = smooth_signal(signal[int(h*frame_len)-100:int(h*frame_len)+100], 7)
        else:
            signal[int(h*frame_len*(1-overlap))-100:int(h*frame_len*(1-overlap))+100] = smooth_signal(signal[int(h*frame_len*(1-overlap))-100:int(h*frame_len*(1-overlap))+100], 7)
    return signal
    ...
    def train_one_epoch(self, in_data, tar_data, up_fr, init_samples, batch_size, optim, loss_func):
        ...
        for batch_i in range(math.ceil(in_data.shape[1] / batch_size)):
            ...
            for k in range(math.ceil((input_batch.shape[0] - init_samples) / up_fr)):
              ...
              loss = loss_func.forward(target_batch[start_i:start_i + up_fr, :,:],output)
              loss.backward()
              optim.step()
...
#%%  TRAINING
    ...
    min_loss = network.train_one_epoch(train_in,train_tar, up_freq, 1000, batch_size,optimiser,loss_functions)
    for epoch in range(EPOCHS):
        ...
        if (epoch_loss.item() < min_loss-0.0001):
            print("Alè!")
            count = 0
...
# %% Define an array of 7 sec of audio that will be given as input to the network
    ...
    tar = np.ones([len(traindata[fs*0:fs*15,0]),1,1])
    inp[:,0,0] = traindata[fs*0:fs*15,0]
    tar[:,0,0] = traindata[fs*0:fs*15,1]
...
#%% Convert the output of the network into a numpy array, savable as a .wav file 
    out = np.zeros([len(traindata),])

    out[0:int(fs*3*(1-overlap))] = output[0:int(fs*3*(1-overlap)),0,0].cpu().numpy()
    for i in range(1,output.shape[1]):
        arr1 = output[int((1-overlap)*output.shape[0]):,i-1,0].cpu().numpy()
        arr2 = output[0:int((overlap)*output.shape[0]),i,0].cpu().numpy()
        arr = (arr1+arr2)/2
        if(overlap < 0.5):
            out[int(i*output.shape[0]*(1-overlap)):int((i+1)*output.shape[0]*(1-overlap))] = np.concatenate((arr,output[int(output.shape[0]*(overlap)):int(output.shape[0]*(1-overlap)),i,0].cpu().numpy()))
        else:
            out[int(i*output.shape[0]*(1-overlap)):int(i*output.shape[0]*(1-overlap))+i*(overlap)*output.shape[0]] = arr
    out[-int(output.shape[0]*(1-overlap)):] = output[int(output.shape[0]*(overlap)):,output.shape[1]-1,0].cpu().numpy()
    out = smooth_frames(out, fs*3, overlap)
...
#%% 
    ...
    sf.write(PATH + "_segm.wav", out, samplerate=44100)
    sf.write(PATH + "_nosegm.wav", output2.cpu().numpy()[:,0,0], samplerate=44100)
#%%
    # %% Save the arrays that we are given to the network as a .wav audio files
    sf.write("in.wav", inp[:,0], samplerate=44100)
    sf.write("tar.wav", tar[:,0], samplerate=44100)
