...
class RNN(nn.Module):
    ...
    def train_one_epoch(self, in_data, tar_data, up_fr, init_samples, batch_size, optim, loss_func):
        ...
        shuffle = shuffle_data(in_data.shape[1],2)
...
if __name__ == "__main__":
    ...
    train_in = split_audio_overlap(traindata[:,0], segm_len,0.5)
    train_tar = split_audio_overlap(traindata[:,1], segm_len,0.5)

    #val_in = split_audio_overlap(validata[:,0], int(fs*3), 0.25)
    #val_tar = split_audio_overlap(validata[:,1], int(fs*3), 0.25)
    val_in = split_audio(validata[:,0], int(fs))
    val_tar = split_audio(validata[:,1], int(fs))
...
# %% Define an array of 7 sec of audio that will be given as input to the network
    inp = np.ones([len(traindata[0:44100,0]),1,1])
    tar = np.ones([len(traindata[0:44100,0]),1,1])

    inp[:,0,0] = traindata[0:44100,0]
    tar[:,0,0] = traindata[0:44100,1]
...
#%% Define the splitted and overlapped array that will be given as input to the network
    ...
    train_split_in = split_audio_overlap(traindata[:,0], segm_len,0.5)
    train_split_tar = split_audio_overlap(traindata[:,1], segm_len,0.5)
...
#%% Convert the output of the network into a numpy array, savable as a .wav file 
    segm_out = np.zeros(len(traindata[:,0]))
    out = output.cpu().numpy()

    for i in range (int(out.shape[1])):
          segm_out[i * int(out.shape[0]*(1 - overlap)):i * int(out.shape[0]*(1 - overlap))+out.shape[0]] = segm_out[i * int(out.shape[0]*(1 - overlap)):i * int(out.shape[0]*(1 - overlap))+out.shape[0]] + out[:,i,0] 
    
    sf.write(PATH + "_nosegm.wav", output2.cpu().numpy()[:,0,0], samplerate=44100)
    sf.write(PATH + "_segm.wav", segm_out, samplerate=44100)
